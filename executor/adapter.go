...
// This function builds an Executor from a plan, if the Executor doesn't returns result,
// Like INSERT, UPDATE statements, it executes within this function, if the Executor returns
// Result, execution work is done after this function, during Next call to the returned ast.RecordSet.
func (a *statement) Exec(ctx context.Context) (ast.RecordSet, error) {
	...
	if len(e.Fields()) == 0 && len(e.Schema()) == 0 {
		// Write statements do not have record set, check if tidb_snapshot ts is set.
		// In history read mode, we can't not do write operations.
		switch e.(type) {
		...
		for {
			...
			// Even thought we don't have result set, but the row is still used to indicates if there is
			// more work to do.
			// For example, UPDATE statement updates a single row for one Next call, we keep calling Next until
