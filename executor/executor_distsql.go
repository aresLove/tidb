...
// lookupTableTask represents a lookup table task.
// It's created from a partial result of an index request, contains the handles in those index keys.
type lookupTableTask struct {
	...
	// handles fetched from index is ordered by index, but to do table request,
	// we need handles be ordered by itself. indexOrder map is used to save the original
	// index order for the handles. Without this map, the original index order would be lost.
...
// rowsSorter sorts the rows by its index order with order map.
...
// There is two kind of execution. one is single-read, in which case we only need to read index keys.
...
// gets distsql.SelectResult which returns multiple distsql.PartialResult, we fetches all the rows from
...
//
// We also make an higher level of concurrency by do index request in a background goroutine, this goroutine
// starts multple worker goroutines, then fetches handles from each index partial request, build lookup table task,
// send the task to worker channel.
//
// Each worker goroutine receives the worker channel to grab a task to do, then executing the task.
// After finished the task, the workers sends the task to a task channel. At the outer most Executor.Next method,
// we receives the finished task, and return each row in that task until no more task to receive.
type XSelectIndexExec struct {
	...
	singleRead bool // for readability.
...
// when the index contains all the needed columns, we only need to do index request.
func (e *XSelectIndexExec) nextForSingleRead() (*Row, error) {
	...
	for {
		...
		if rowData == nil {
			// Finished current partial result, get next one.
			e.partialResult = nil
			continue
		}
		if e.aggregate {
...
// DoubleRead XSelectIndexExec first
...
// It works like a executing a XSelectTableExec, except that the ranges is built from a slice of handles
// rather than table ranges. it sends the request to all the regions contains those handles.
...
type XSelectTableExec struct {
	...
	result distsql.SelectResult
	// each row is fetched by partial result.
	partialResult distsql.PartialResult
