...
// Row represents a result set row, it may be returned from a table, a join, or projection.
type Row struct {
...
// RowKeyEntry contains row key handle and the table it comes from and the table's alias name.
...
// For "SELECT .. FOR UPDATE" statement, it locks every row keys from source Executor.
// After the execution, the keys are bufferred in transaction, the lock will sent to KV
// when doing commit. If there are any keys already locked by another transaction,
// The transaction will fail to commit and retry.
...
// Every distinct row will be added to the map, the memory usage can be very high.
...
func init() {
	// In plan package we need to execute non-correlated subquery to do optimization,
	// but plan package can not import executor package because of dependency cycle.
	// So we assign a function to plan package using executor implementation to avoid dependency cycle.
...
// makeJoinRow simply create a new row that append row b to row a.
...
// fillNulRow creates a null filled result row from a big row.
// For outer join, when a row from outer table doesn't has any matched rows, this is need.
...
type HashSemiJoinExec struct {
	...
	otherFilter  expression.Expression
	schema       expression.Schema
	// withAux mean the result row needs to add an extra column to store a bool or null value.
	withAux           bool
	targetTypes       []*types.FieldType
...
func (e *HashSemiJoinExec) Next() (*Row, error) {
	...
	for {
		...
		// For withAux subquery, we returns the row with a Datum indicates if it's match,
		// For non-withAux subquery, we only returns the matched row.
		if e.withAux {
			if isNull {
...
// TopnExec implements a top n algorithm, it keeps the top-n element in a heap, to reduce memory usage.
	...
	if !e.fetched {
		...
		for {
			...
			if e.totalCount == e.heapSize {
				...
				// to reduce the number of comparison time.
				e.Rows = append(e.Rows, orderRow)
...
// TrimExec truncates the number of columns in src rows.
// Some columns in src rows are not needed in the result.
// For example 'SELECT a from t order by b'.
// In this case, b needed for ordering, but we don't need b in the result.
